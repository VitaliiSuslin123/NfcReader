        ...
        if (byteArray == null) {
            ...
            if (startPos >= 0 && byteArray.length >= startPos + length) {
                ...
                for (int i = 0; i < length; ++i) {
                    value += (byteArray[startPos + i] & DEFAULT_MASK) << 8 * (length - i - 1);
                }
    ...
    private static String formatByte(byte[] pByte, String pFormat, boolean pTruncate) {
        ...
        if (pByte == null) {
            ...
            for (int i$ = 0; i$ < len$; ++i$) {
                ...
                if (b != 0 || !pTruncate || t) {
                    t = true;
                    sb.append(String.format(pFormat, new Object[]{Integer.valueOf(b & DEFAULT_MASK)}));
                }
            }
        }
        ...
        if (pData == null) {
            ...
            if (text.length() % 2 != 0) {
                ...
                for (int i = 0; i < text.length(); i += 2) {
                    Integer val = Integer.valueOf(Integer.parseInt(text.substring(i, i + 2), HEXA));
